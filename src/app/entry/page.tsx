...
function AdminContent() {
  ...
  const addProduct = async () => {
    if (!newProductId.trim()) {
      setMessage('Bitte geben Sie eine Produkt-ID ein');
      return;
    }
    ...
    try {
      const response = await fetch('/api/products', {
        ...
        body: JSON.stringify({ amazonId: newProductId.trim() }),
      });
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex space-x-3">
                      <Input
                        placeholder="z.B. B08N5WRWNW (von amazon.de)"
                        value={newProductId}
                      ...
                      <Button
                        onClick={addProduct}
                        disabled={isLoading}
                        className="bg-primary-green hover:bg-green-600 text-white px-8"
                      >
                        {isLoading ? 'Hinzufügen...' : 'Hinzufügen'}
                      </Button>
                    </div>
                    <p className="text-sm text-gray-500 mt-2">
                      Geben Sie die Produkt-Identifikationsnummer ein, die Sie in der Produkt-URL finden
                    </p>
                  </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 className="font-medium text-blue-800 mb-2">So finden Sie die Produkt-ID (ASIN):</h4>
                    <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
                      <li>Gehen Sie zu einer beliebigen Produktseite</li>
                      <li>Schauen Sie in die URL: amazon.de/dp/<strong>B08N5WRWNW</strong></li>
                      <li>Kopieren Sie den 10-stelligen Code nach "/dp/"</li>
                      <li>Fügen Sie ihn oben ein und wir holen automatisch die Produktdetails ab</li>
                    </ol>
                    <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded">
                      <p className="text-sm text-green-700">
                        ✅ <strong>Referral-Code wird automatisch hinzugefügt:</strong> ?tag=50674-21
                      </p>
                    </div>
                  </div>
                </div>
